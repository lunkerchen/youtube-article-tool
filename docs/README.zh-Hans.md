...
**🎨 UI/UX**
- **data-i18n 属性系统**：将脆弱的 DOM querySelector 链改为 `data-i18n="key"` — 语言切换更稳健、快速、可扩展。
- **统一 Toast 通知系统**：移除独立的错误显示区块，所有通知（信息/成功/错误）使用动画 Toast，附自动消失进度条 + 悬停暂停。
- **SVG 图标取代 Emoji**：所有 emoji 换成 inline SVG 图标，跨平台渲染一致。
- **Skeleton 骨架加载**：处理期间显示 shimmer 占位符，取代空白画面。
- **API Key 显示/隐藏切换**：眼睛图标开关。
- **Tab 键盘导航**：方向键 + Home/End 切换多结果标签页，附 focus ring。
- **配置卡片重构**：API Key、URL 输入、语言设定清楚分区。
- **Sticky 顶部导航栏**：语言选择器移至固定顶栏，手机上不再重叠。
- **复制按钮反馈**：点击后按钮变绿显示「已复制 ✓」1.5 秒。
- **回到顶部按钮**：滚动 400px 后浮现。
- **Loading Spinner**：转换按钮处理中显示旋转动画 +「转换中...」。
- **Enter 快捷键**：在 URL 输入框按 Enter 即可转换。
- **主题系统**：2 种主题 — 深色（默认）与高对比。localStorage 持久化。
- **时间分组历史记录**：今天 / 昨天 / 本周 / 更早。
- **自动侦测浏览器语言**：首次访问自动比对 `navigator.language`。
- **提升基础字体大小**：所有文字加大一级，提升阅读性。
...
### v3.4 (2026-05-01) — 元数据提取弹性强化

**🐛 Bug 修复**
- **BrokenPipeError 自动重试**：将 `--dump-json` 提取独立为 `get_video_meta()` 函数，具备 3 次自动重试机制（1s/2s/3s 递增间隔）。此前长视频（118KB+ 元数据）的间歇性 pipe 中断会导致整个转换失败，现在会自动恢复。
- **临时文件保证清理**：`task_temp` 与 `meta_file` 清理移至 `finally` 区块。即使发生异常，临时文件也会可靠移除。
- **内存效率优化**：将写文件再读回的冗余流程（`stdout.decode()` → 写文件 → `json.load()`）改为直接 `json.loads()` 处理捕获的 stdout，减少 I/O 与解码往返。
- **统一 `run_with_retry()`**：将所有 `subprocess.run` 调用重构为统一的 `run_with_retry()` 包装器，具备自动 BrokenPipeError 重试（3 次、线性 backoff）。此前只有 `--dump-json` 有 retry 保护，现在字幕下载（含 `--sub-langs all` 重试）也具备 pipe 中断自愈能力。
- **移除最后一次尝试的浪费 sleep**：最终失败后不再多等 3 秒。
...
- **`[Errno 32] Broken pipe` 的实际原因**：当 `yt-dlp --dump-json` 输出大量元数据时（长视频可达 118KB+），管道可能因数据量过大而中断。v3.4 引入专用 `get_video_meta()` 函数，具备 3 次自动重试与递增延迟机制，pipe 中断会自动恢复而非导致转换失败。若此错误与 429 同时出现，代表字幕下载阶段也受影响，更新至 v3.3 以上即可。
