...
### v3.6 (2026-06-20) — 可攜式、測試與 MCP

**🔧 改善**
- **可攜式路徑**：將所有硬編碼的絕對路徑 (`/Users/lunker/...`) 替換為 `os.path.dirname`，clone 到任何位置都能正常運作。
- **49 個單元測試**：完整覆蓋字幕清洗 (VTT/SRT/JSON)、歷史紀錄 CRUD、重試邏輯、縮圖回退、字幕語言選擇。
- **MCP 伺服器**：`mcp_server.py` 透過 MCP 協議暴露轉換工具，支援 Hermes Agent / Claude Desktop 整合。
- **Docker 支援**：`Dockerfile` 一鍵容器化部署。
- **CORS 中間件**：本地開發便利性提升。
- **Google API Key 備援**：`get_api_key()` 現支援 `GOOGLE_API_KEY` 環境變數作為備援。
- **LICENSE**：新增 MIT 授權條款。

### v3.5 (2026-05-02) — 全面 UI/UX 改版

**🎨 UI/UX**
- **data-i18n 屬性系統**：將脆弱的 DOM querySelector 鏈改為 `data-i18n="key"` — 語言切換更穩健、快速、可擴展。
- **統一 Toast 通知系統**：移除獨立的錯誤顯示區塊，所有通知（資訊/成功/錯誤）使用動畫 Toast，附自動消失進度條 + 懸浮暫停。
- **SVG 圖標取代 Emoji**：所有 emoji 換成 inline SVG 圖標，跨平台渲染一致。
- **Skeleton 骨架載入**：處理期間顯示 shimmer 佔位符，取代空白畫面。
- **API Key 顯示/隱藏切換**：眼睛圖標開關。
- **Tab 鍵盤導航**：方向鍵 + Home/End 切換多結果分頁，附 focus ring。
- **配置卡片重構**：API Key、URL 輸入、語言設定清楚分區。
- **Sticky 頂部導航欄**：語言選擇器移至固定頂欄，手機上不再重疊。
- **複製按鈕回饋**：點擊後按鈕變綠顯示「已複製 ✓」1.5 秒。
- **回到頂部按鈕**：滾動 400px 後浮現。
- **Loading Spinner**：轉換按鈕處理中顯示旋轉動畫 +「轉換中...」。
- **Enter 快捷鍵**：在 URL 輸入框按 Enter 即可轉換。
- **主題系統**：2 種主題 — 深色（預設）與高對比。localStorage 持久化。
- **時間分組歷史紀錄**：今天 / 昨天 / 這週 / 更早。
- **自動偵測瀏覽器語言**：首次訪問自動比對 `navigator.language`。
- **提升基礎字體大小**：所有文字加大一級，提升閱讀性。

### v3.4 (2026-05-01) — 元數據提取彈性強化
...
本工具可作為 MCP (Model Context Protocol) 伺服器運行，將轉換工具暴露給 Hermes Agent、Claude Desktop 等 AI 助理。
...
**API Key 驗證 (HTTP 模式)：**
設定 `MCP_API_KEYS=sk-a1b2c3,sk-d4e5f6` 管理多把金鑰。客戶端需帶 `Authorization: Bearer <key>` 標頭。
...
│   └── README.ja.md         # 日文文件
├── tests/
│   └── test_main.py          # 49 個單元測試
├── mcp_server.py             # MCP 伺服器 (AI Agent 整合)
├── Dockerfile
├── LICENSE                   # MIT
├── start.sh                 # 快速啟動腳本
...
- **`[Errno 32] Broken pipe` 的真實原因**：當 `yt-dlp --dump-json` 輸出大量元數據時（長影片可達 118KB+），管道可能因資料量過大而中斷。v3.4 引入專用 `get_video_meta()` 函數，具備 3 次自動重試與遞增延遲機制，pipe 中斷會自動恢復而非導致轉換失敗。若此錯誤與 429 同時出現，代表字幕下載階段也受影響，更新至 v3.3 以上即可。
