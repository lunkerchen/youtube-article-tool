...
- **v3.2**
  - **UI/UX 深度優化**：重構介面佈局為「設定」與「輸入」兩大區塊，並加入 API Key 本地儲存的安全提示。
  - **智慧連結解析**：強化 URL 辨識邏輯，支援包含逗號、空格等多種分隔符號的內容輸入。
  - **多影片分頁管理**：結果區域新增互動分頁標籤（Tabs），方便管理批次轉換產出的多篇文章。
  - **生產力增強**：加入 `Ctrl/Cmd + Enter` 快捷鍵啟動轉換，並新增「清除輸入」按鈕。
- **v3.1**
  - 新增簡體中文 UI 介面支持。
  - 優化複製 Markdown 的用戶體驗，改為 Toast 通知。
  - 重構專案目錄結構並優化圖片 fallback 機制。
